...
func CheckJWT() gin.HandlerFunc {
	...
	if err != nil {
		logger.Error("Failed to set up the validator", slog.String("error", err.Error()))
		os.Exit(1)
		//log.Fatalf("failed to set up the validator: %v", err)
	}

	errorHandler := func(w http.ResponseWriter, r *http.Request, err error) {
		logger.Error("Encountered error while validating JWT", slog.String("error", err.Error()))
...
func GetCustomClaims(ctx *gin.Context) *CustomClaims {
	...
	logger.Info("Custom Claims Name", slog.String("message", customClaims.Name))
	logger.Info("Custom Claims CA", slog.String("message", customClaims.CA))
